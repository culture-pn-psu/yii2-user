...
class Ldap {
    ...
    public function UserInfo($ldap) {
        $sr = ldap_search(
                $ldap, $this->basedn, "(&(objectClass=user)(objectCategory=person)(sAMAccountName=" . $this->username . "))", array("cn", "dn", "samaccountname", "employeeid", "citizenid", "company", "campusid", "department", "departmentid", "physicaldeliveryofficename", "positionid", "description", "displayname", "title", "personaltitle", "personaltitleid", "givenname", "sn", "sex", "userprincipalname", "mail")
        );

        $info = ldap_get_entries($ldap, $sr);

        $fullname = explode(' ', $info[0]["description"][0]);
        $firstname = array_shift($fullname);
        $name_last = array_pop($fullname);
        $name_middle = trim(implode(' ', $fullname));
        $lastname = $name_middle ? @implode(' ', [$name_middle, $name_first]) : $name_last;
